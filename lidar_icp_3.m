...
[mouse_gyro_times, ~, ~, ~, ~, ~, angleOverTimeMG] = get_mouse_gyro_pos('0', num2str(realmax));


for i=2:1:length(decoded_data)
    if subidxrmse > 20
        subidxrmse = 0;
        disp('Warning: resetting rmse sub, too large');
    end

    data1 = decoded_data{i-1-subidxrmse};
    data2 = decoded_data{i};

    ptCloud1 = createPointCloud(data1);
    ptCloud2 = createPointCloud(data2);

    [tform, ~, rmse] = pcregistericp(ptCloud1, ptCloud2);
    rot = tform.R;
