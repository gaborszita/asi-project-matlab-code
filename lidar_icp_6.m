function [lidar_times, xTransform, yTransform, angleTransform, xOverTime, yOverTime, angleOverTime, rmseOverTime] = lidar_icp_6(logfilename, time1, time2)
...
for i=2:1:length(decoded_data)
    ...
    xInitialTransformStart = -0.20;
    xInitialTransformEnd = 0.20;
    xInitialTransformStep = 0.02;
    yInitialTransformStart = -0.20;
    yInitialTransformEnd = 0.20;
    yInitialTransformStep = 0.02;
    angleInitialTransformStart = 0;
    angleInitialTransformEnd = 90;
    angleInitialTransformStep = 5;
    % iterate through all possible initial transforms, find the rmse for each, then choose the one with the least rmse
    minRmse = 1000000000;
    angleTransformCurrent = angleTransform;
    for xInitialTransform=xInitialTransformStart:xInitialTransformStep:xInitialTransformEnd
        for yInitialTransform=yInitialTransformStart:yInitialTransformStep:yInitialTransformEnd
               angleInitialTransform=0;
                rotationMatrix = [cosd(angleInitialTransform), -sind(angleInitialTransform), 0;
                       sind(angleInitialTransform), cosd(angleInitialTransform), 0;
                       0, 0, 1];
    
                rigidTransformInitial = rigid3d(rotationMatrix, [xInitialTransform yInitialTransform 0]);
                %rigidTransformInitial = rigid3d(rotationMatrix, [0 0 0]);

                [tform, ~, rmse] = pcregistericp(pcdenoise(ptCloud1), pcdenoise(ptCloud2), MaxIterations=100000,Tolerance=[0.00001, 0.00001], InlierRatio=0.995, InitialTransform=rigidTransformInitial);
                %rmse
                %minRmse
                if rmse < minRmse
                    disp('less');
                    ...
                    angleTransformCurrent = angleTransform + eulerAnglesDeg(1);
    ...
    angleTransform = angleTransformCurrent;
